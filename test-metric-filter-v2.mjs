    ...
    try {
        ...
        if (standardAnodes.length > 0) {
            ...
            for (let i = 0; i < Math.min(3, standardAnodes.length); i++) {
                ...
                // Check for size in the name itself since we simplified names
                const hasInch = name.match(/\d+\/\d+"|[\d.]+"\s/);
                const hasMm = name.match(/\d+mm/i);
                console.log(`    - "${name}" ${hasInch ? '✅ Inch' : hasMm ? '❌ Has mm' : '⚠️ Check size'}`);
            }
        }
        ...
        if (metricAnodes.length > 0) {
            ...
            for (let i = 0; i < Math.min(3, metricAnodes.length); i++) {
                ...
                const hasMm = name.match(/\d+mm/i);
                const hasInch = name.match(/\d+\/\d+"|[\d.]+"\s/);
                console.log(`    - "${name}" ${hasMm ? '✅ Metric' : hasInch ? '❌ Has inch' : '⚠️ Check size'}`);
            }
        }
        ...
        const filterWorking = (standardAnodes.length > 0) &&
                            (metricAnodes.length > 0) &&
        ...
        if (!filterWorking) {
            if (standardAnodes.length === 0) {
                console.log('  ⚠️ No standard anodes found - filter may be too restrictive');
            }
            if (metricAnodes.length === 0) {
                console.log('  ⚠️ No metric anodes found - filter may be too restrictive');
            }
            if (standardAnodes.length === allShaftAnodes.length) {
                console.log('  ⚠️ Standard filter showing all anodes - not filtering');
            }
            if (metricAnodes.length === allShaftAnodes.length) {
                console.log('  ⚠️ Metric filter showing all anodes - not filtering');
            }
        }
